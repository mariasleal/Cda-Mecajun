...
<html lang="pt-br">
	...
	<?php
		...
		if (isset($_POST['digbutton'])){
				...
				$result = pg_query($query) or die('Erro novo1!');
				$query2 = "SELECT * FROM digital WHERE dig_troca != '-1'";
				$result2 = pg_query($query2) or die('Erro novissimo!');
				if (pg_affected_rows($result)>0 || pg_affected_rows($result2)>0){
					echo "<script language='javascript' type='text/javascript'>alert('Digital pendente para ser cadastrada ou atualizada!');window.location.href='edit.php';</script>";
				}
				else{
					$query1 = "SELECT dig_pos FROM membros where id = '" . $_COOKIE['id'] . "'  ";
					$result1 = pg_query($query1) or die('Erro novissimo!');
					$line = pg_fetch_array($result1, null, PGSQL_ASSOC);
					$query = "UPDATE digital SET dig_troca = '" . $line["dig_pos"] . "' WHERE dig_troca = '-1'";
					$result = pg_query($query) or die('Erro novo2!');
					$query = "UPDATE membros SET dig_pos = '-1' WHERE id = '" . $_COOKIE['id'] . "' ";
					$result = pg_query($query) or die('Erro novo3!');
		...
		if(isset($_POST['add'])){
			...
			if(!empty($_POST['horain']) && !empty($_POST['minin']) && !empty($_POST['horaout']) && !empty($_POST['minout']) && isset($_POST['Day'])){
				/*echo "foi sim!";*/
				$query = "insert into horarios_fixos values(" . $_COOKIE['id'] . ",'" . $_POST['Day'] . "','" . $_POST['horain'] . "','" . $_POST['minin'] . "','" . $_POST['horaout'] . "','" . $_POST['minout'] . "')"; 
		...
		elseif(isset($_POST['onlyone'])){
			...
			$query = "SELECT * FROM horarios_fixos WHERE id = " . $_COOKIE['id'] . " ORDER BY dia, horain, minin, horaout, minout";
			$result = pg_query($query) or die('Erro1!');
			...
			/* Remove o horario */
			$query = "DELETE FROM horarios_fixos WHERE id = " . $_COOKIE['id'] . " AND dia = '" . $dia . "' AND horain = '" . $horain . "' AND minin = '" . $minin . "' AND horaout = '" . $horaout . "' AND minout = '" . $minout . "'";
			$result = pg_query($query) or die('Erro2!');
			...
			$query = "DELETE FROM horarios_fixos WHERE id = " . $_COOKIE['id'];
			/*echo $query;*/
			...
			$query = "SELECT * FROM horarios_fixos WHERE id = " . $_COOKIE['id'] . " ORDER BY dia, horain, minin, horaout, minout";
			$result = pg_query($query) or die('Erro3!');
			...
			$result = pg_query($query) or die('Erro4!');
			...
			$query = "select * from horarios_fixos where id = " . $idfunc;
			$result = pg_query($query) or die('Erro!');
			...
			$result = pg_query($query) or die('Erro5!');
